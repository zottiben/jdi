...
Quick scan: directory structure (`tree -L 2 -d`), file counts by type, package files (package.json, composer.json), config files.
...
Map directory structure, entry points, layers (controllers, services, models), data flow, external integrations. Output to `ARCHITECTURE.md`.

### Step 4: Convention Analysis

Document naming conventions (files, functions, classes), import patterns, component patterns, coding style. Output to `CONVENTIONS.md`.
...
Combine into `SUMMARY.md` with quick reference table (language, framework, test coverage, key concerns), key findings (strengths, concerns, recommendations), and links to detailed files.

### Step 8: Save Analysis

Write all files to `.jdi/codebase/`:
- `SUMMARY.md` — Quick reference
- `STACK.md` — Technology stack
- `ARCHITECTURE.md` — System structure
- `CONVENTIONS.md` — Coding patterns
- `TESTING.md` — Test approach
- `CONCERNS.md` — Risk areas
