...
```
You are {agent-name}. Read .claude/jedi/agents/{spec}.md and .claude/jedi/components/meta/AgentBase.md.

TEAM: {team-name}
PLAN: {plan-path}
WORKING_DIR: {working-directory}

Claim tasks from TaskList assigned to you. For each:
1. Read task description for files/objective/steps
2. Implement using Edit tool
3. SendMessage to coordinator with structured return
